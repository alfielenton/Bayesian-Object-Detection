...
class BayesClassifier:
    ...
    def predict_cats(self, x):
        ...
        M = self.Mq
        Sigma = (torch.exp(self.log_epsq) / (torch.exp(self.log_tauq) - 1)) * (1 + torch.exp(self.log_Uq + self.log_Vq))
    ...
    def parameters_step(self, batch):
        ...
        exp_log_q_M = .5 * (self.N * self.C) * (1 + math.log(2 * torch.pi)) \
                      + .5 * self.C * self.log_Uq + .5 * self.N * self.log_Vq
        ...
        exp_log_p_M = -.5 * self.N * self.C * math.log(2 * torch.pi) - .5 * self.C * self.log_Uq - .5 * self.N * self.log_Vq \
        ...
        exp_log_like = -D * math.log(self.C) -.5 * D * self.N * math.log(2 * torch.pi) \
                       -.5 * self.N * (torch.log(eps_cd) - torch.digamma(tau_cd)).sum() \
                       -.5 * ((tau_cd / eps_cd) * (sq_dist + self.N * torch.exp(self.log_Uq + self.log_Vq))).sum()
